...
impl Png {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut res = Vec::new();
        res.append(&mut Png::STANDARD_HEADER.to_vec());

        for val in self.chunks.iter() {
            res.append(&mut val.as_bytes())
        }
        res
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        let (head, chunk) = bytes.split_at(8);
        match head.to_vec() == Png::STANDARD_HEADER {
            true => {}
            false => return Err(Box::from("head err")),
        }
        ...
        loop {
            ...
            match right.len() {
                0 => break,
                x if x < 12 && x > 0 => return Err(Box::from("chunk err")),
                _ => chunks = right,
            }
        }
